...
- [`on.exit()`](https://rdrr.io/r/base/on.exit.html) expressions and
  stack-based cleanup such as provided by `local_` functions in the
  [withr](https://github.com/r-lib/withr/) package
- Step-debugging and [`browser()`](https://rdrr.io/r/base/browser.html)
  within coroutines
...
a file, computing a value in an external process), use
[`await()`](https://coro.r-lib.org/dev/reference/async.md). The argument
to [`await()`](https://coro.r-lib.org/dev/reference/async.md) must
...
The [`generator()`](https://coro.r-lib.org/dev/reference/generator.md)
...
[`await()`](https://coro.r-lib.org/dev/reference/async.md) can be used
in loops, if/else branches,
[`tryCatch()`](https://rdrr.io/r/base/conditions.html) expressions, or
any combinations of these. However they can’t be used as function
arguments. These will cause errors:
...
Despite this transformation of source code,
[`browser()`](https://rdrr.io/r/base/browser.html) and step-debugging
still work as you would expect. This is because coro keeps track of the
source references from the original code.
